...
try {
    ...
    // Vérification que $res_num est valide
    if ($res_num <= 0) {
        throw new Exception("Numéro de réservation invalide.");
    }

    // Redirection absolue vers reservation_success.php
    $success_url = "reservation_success.php?ticket=" . urlencode($res_num);
    header("Location: " . $success_url);
    exit;
} catch (Exception $e) {
    // En cas d'erreur, afficher le message (temporaire pour debug)
